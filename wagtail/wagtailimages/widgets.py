...
class AdminMultipleImageChooser(AdminChooser):
    choose_one_text = _('Choose Multiple images')
    choose_another_text = _('Choose another image')
    link_to_chosen_text = _('Edit this image')

    def __init__(self, **kwargs):
        super(AdminMultipleImageChooser, self).__init__(**kwargs)
        self.image_model = get_image_model()

    def render_html(self, name, value, attrs):
        instance, value = self.get_instance_and_id(self.image_model, value)
        original_field_html = super(AdminMultipleImageChooser, self).render_html(name, value, attrs)
    ...
    def render_js_init(self, id_, name, value):
        return "createMultipleImageChooser({0});".format(json.dumps(id_))
