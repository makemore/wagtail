...
from .widgets import AdminImageChooser, AdminMultipleImageChooser
...
class BaseMultipleImageChooserPanel(BaseChooserPanel):
    object_type_name = "image"

    @classmethod
    def widget_overrides(cls):
        return {cls.field_name: AdminMultipleImageChooser}
...
class MultipleImageChooserPanel(object):
    def __init__(self, field_name):
        self.field_name = field_name

    def bind_to_model(self, model):
        return type(str('_ImageChooserPanel'), (BaseMultipleImageChooserPanel,), {
            'model': model,
            'field_name': self.field_name,
        })
